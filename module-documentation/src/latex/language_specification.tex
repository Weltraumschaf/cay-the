...
To make the language less noisy the design goal is to add only the essential necessary things. One of the most anoying noise in lots of languages are unnecessary semicolons and braces. So in consequence this language makes them obsolete.

\subsecrion{No Semicolons}

Semicolons were introduced centuries ago to delimit statements to make it possible to write multiple statements in one line. But this is considered a bad practize nowadays because it leads to unreadable code. If there is only one statement allowed per line a semicolon is  ot needed as delomiter and we can rely on a simple newline as delimiter. So in Cay-The statements are delimited by newlines.
