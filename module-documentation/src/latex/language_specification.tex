...
\begin{itemize}
    \item \texttt{private}: Only visible inside the declaring type.
    \item \texttt{protected}: Visible to all delegatees.
    \item \texttt{package}: Visible inside the same package.
    \item \texttt{public}: Visible to everything inside the module.
    \item \texttt{export}: Visible outside the module.
\end{itemize}

Default visibility is always \texttt{private}. Even if a type is declared with weaker visibility than \texttt{private} this does not automatically apply to all methods and properties of this type. All of them are \texttt{private} by default and visibility must be changed to something weaker respectively.
